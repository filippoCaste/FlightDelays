...
public class ExtFlightDelaysDAO {
	...
	public List<Airport> getVertici(int x, Map<Integer, Airport> idMap) {
		...
		try {
			...
			while (rs.next()) {
				result.add(idMap.get(rs.getInt("ID"))); // la mappa ha tutti gli aeroporti quindi non serve fare controlli
			}
	...
	public List<Rotta> getRotte(Map<Integer, Airport> idMap) {
		String sql = "SELECT f.ORIGIN_AIRPORT_ID AS id1, f.DESTINATION_AIRPORT_ID AS id2, COUNT(*) AS nvoli "
				+ "FROM flights f "
